    ...
    if !(filename in sols)
        ...
        for u in control_signals
            for ω in ω_range
                ...
                #full_prod[control_labels[u],ω] = [analytical_prod(u, t, ω, W, Λ, Winv, Bin, ME.F) for t in t_range]
                full_prod[control_labels[u],ω] = [Float64.(evaluate_prod(BigFloat.(full_sol[control_labels[u],ω][i]), u, BigFloat(t_range[i]), BigFloat(ω), W, Λ, Winv, Bin, ME.F)) for i in eachindex(t_range)]
            end
        end
